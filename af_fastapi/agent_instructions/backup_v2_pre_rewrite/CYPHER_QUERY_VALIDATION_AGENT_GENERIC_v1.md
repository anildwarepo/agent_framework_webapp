...
### 0.3.1 Comment Stripping (BLOCKING — always do this FIRST before anything else)
Before any other processing, you MUST strip ALL `//` comment lines and `/* */` blocks from inside the Cypher body (between the `$$` delimiters). AGE does not support comments — queries containing them WILL fail with syntax errors.

**This includes:**
- Single-line comments: `// any text`
- Block comments: `/* any text */` (including multi-line and emoji-containing like `/* 1️⃣ ... */`)
- Inline comments at end of lines

**Verify after stripping:** Scan the Cypher body one more time. If any `//` or `/*` remains, strip it again. Only proceed to validation after confirming the Cypher body is comment-free.

**Your corrected FINAL_SQL output MUST also be comment-free.** Never output a query that contains comments.
...
Checks:
- Graph name correct
- `ag_catalog.cypher(` present (NOT bare `cypher(` — missing `ag_catalog.` prefix causes errors)
- `$$` delimiters matched
- `AS (...)` present
- RETURN count = AS column count
- All columns typed `ag_catalog.agtype` (not `text`, `bigint`, `jsonb`, `integer`, and not bare `agtype`)
- Ends with `;`
- No nested `$$`
- Outer SELECT is `SELECT *` — do NOT place Cypher functions (`labels()`, `type()`, `id()`) in outer SQL SELECT
- **No comments** (`//` or `/* */`) inside the `$$` Cypher body

**WRONG:** `FROM cypher('graph', $$ ... $$) AS (col agtype);`
**CORRECT:** `FROM ag_catalog.cypher('graph', $$ ... $$) AS (col ag_catalog.agtype);`
...
| `exists(node.prop)` (property existence check) | `node.prop IS NOT NULL` |
| `CALL` subqueries / `CALL db.labels()` | Not supported in AGE — use `MATCH (n) RETURN labels(n), count(*)` for label discovery |
| `unnest()` on agtype columns in outer SQL | Not supported — keep all processing inside Cypher body, not outer SQL |
